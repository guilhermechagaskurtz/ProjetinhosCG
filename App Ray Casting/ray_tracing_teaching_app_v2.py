...
class RayTeachingApp:
    ...
    def build_ui(self):
        ...
        right = ttk.Frame(content, style='Card.TFrame', width=380)
    ...
    def build_controls(self, parent):
        ...
        mode_combo.bind('<<ComboboxSelected>>', lambda e: self.draw_everything())
        ...
        for text, var in [
            ...
            ttk.Checkbutton(render_box, text=text, variable=var, command=self.draw_everything).pack(anchor='w', padx=10, pady=(8 if text == 'Iluminação local' else 0, 0))

        self.add_labeled_scale(render_box, 'Recursões máximas', self.max_depth_var, 0, 6, self.draw_everything)
        self.add_labeled_scale(render_box, 'Samples por pixel (Path)', self.path_samples_var, 1, 24, self.draw_everything)
        self.add_labeled_scale(render_box, 'Bounces máximos (Path)', self.path_bounces_var, 1, 8, self.draw_everything)
        ...
        self.add_labeled_scale(grid_box, 'Largura do plano', self.plane_w_var, 3.0, 10.0, self.draw_everything, resolution=0.1)
        self.add_labeled_scale(grid_box, 'Altura do plano', self.plane_h_var, 2.0, 8.0, self.draw_everything, resolution=0.1)
