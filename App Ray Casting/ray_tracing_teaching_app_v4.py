...
class RayTeachingApp:
    ...
    def make_state(self):
        ...
        self.live_selected_pixel_var = tk.BooleanVar(value=True)
        self.live_full_grid_var = tk.BooleanVar(value=False)
    ...
    def build_controls(self, parent):
        ...
        buttons.pack(fill='x', padx=10, pady=(4, 6))
        ...
        live_box = tk.Frame(grid_box, bg='#121936')
        live_box.pack(fill='x', padx=10, pady=(0, 10))
        ttk.Checkbutton(
            live_box,
            text='Atualizar pixel selecionado em tempo real',
            variable=self.live_selected_pixel_var,
            command=self.on_live_toggle_change
        ).pack(anchor='w')
        ttk.Checkbutton(
            live_box,
            text='Atualizar grid inteira em tempo real',
            variable=self.live_full_grid_var,
            command=self.on_live_toggle_change
    ...
    def on_live_toggle_change(self):
        if self.live_full_grid_var.get():
            self.live_selected_pixel_var.set(True)
    ...
    def update_info_label(self):
        if self.last_trace_active and self.last_trace_pixel is not None and self.last_trace_color is not None:
            ...
            suffix = ''
            if self.live_full_grid_var.get():
                suffix = ' | atualização ao vivo: grid inteira'
            elif self.live_selected_pixel_var.get():
                suffix = ' | atualização ao vivo: pixel selecionado'
    ...
    def recompute_live_trace(self):
        ...
        if not self.last_trace_active or self.last_trace_pixel is None:
            self.draw_everything()
            return
        ...
        if self.live_selected_pixel_var.get() or self.live_full_grid_var.get():
